...
export default function MorningMotivation() {
  ...
  useEffect(() => {
    if (!user?._id || !user?.morningMotivation) return;
    const today = new Date().toISOString().slice(0, 10);
    const seenKey = `morning-seen-${user._id}`;
    const contentKey = `morning-content-${user._id}`;
    ...
    api
      ...
      .then((res) => {
        ...
        localStorage.setItem(contentKey, res.data.content);
      })
      .finally(() => setLoading(false));
  }, [user?._id, user?.morningMotivation]);
